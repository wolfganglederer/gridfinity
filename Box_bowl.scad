...
difference(){
    union(){
        ...
        translate([0,0,(gridz)*7])resize([gridx*42-4,gridy*42-4 ,(gridz-1)*7*2])sphere(r = 1, $fn = 100);
    translate([0,0,gridz*7 +  2 ])cube([gridx*42, gridy*42, 4], center = true);
    translate([0,0,gridz*7])
    difference(){
        cube([gridx*42+1, gridy*42+1, 4], center = true);
        
        translate([0,0,-3])
            minkowski(){
            cube([gridx*42-4, gridy*42-4, 0.1], center = true);
            sphere(d=5);
        }
}
}
